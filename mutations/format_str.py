...
from special_values import INJECTION_PAYLOADS, BOUNDARY_VALUES
...
def format_injection(data: bytes) -> Iterator[bytes]:
    ...
    format_specifiers = [
        ...
        b"%p %p %p" * 100,  # Multiple pointers; tests memory leak potential with several pointers in sequence.
        b"%s %s %s" * 100,  # Multiple pointers; tests memory leak potential with several pointers in sequence.
        b"%n",
