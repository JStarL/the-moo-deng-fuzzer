...
def metadata_mutation(data: bytes) -> Iterator[bytes]:
    """
    Mutates PDF metadata by injecting boundary values, oversized strings, invalid keys, and random data.
    This generator yields mutated PDF binaries designed to test parser robustness.

    Parameters:
    - data (bytes): The binary data of the original PDF file.

    Returns:
    - Iterator[bytes]: An iterator over mutated PDF binaries.
    """

    # Helper function to yield mutated document as bytes
    def yield_mutated_doc_bytes(doc, metadata):
    ...
    yield from yield_mutated_doc_bytes(doc, nested_metadata)
    ...
    yield from yield_mutated_doc_bytes(doc, control_metadata)
    ...
    yield from yield_mutated_doc_bytes(doc, large_integers_metadata)
    ...
    yield from yield_mutated_doc_bytes(doc, binary_metadata)
    ...
    yield from yield_mutated_doc_bytes(doc, invalid_keys_metadata)
    ...
    yield from yield_mutated_doc_bytes(doc, alternating_metadata)
